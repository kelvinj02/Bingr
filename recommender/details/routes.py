...
from recommender.api_clients.movies_client import get_movie_recommendations
...
def detail(item_type, item_id):
    if item_type == 'movie':
        item = get_movie_recommendations(item_id)
    elif item_type == 'book':
        item = get_book_recommendations(item_id)
    else:
        abort(404)
